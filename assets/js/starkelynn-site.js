(function () {
  ...
  function initBadgeOrbit() {
    var orbits = Array.prototype.slice.call(document.querySelectorAll("[data-badge-orbit]"));
    ...
    function normalizeDuration(value, fallback) {
      ...
      return value.trim().slice(-2) === "ms" ? parsed : parsed * 1000;
    }

    function setupOrbit(orbit) {
      var rail;
      var traces;
      var badge;
      var styles;
      ...
      if (orbit.getAttribute("data-orbit-ready") === "true") {
        return;
      }

      orbit.setAttribute("data-orbit-ready", "true");
      rail = orbit.querySelector(".site-footer__badge-orbit-rail");
      traces = Array.prototype.slice.call(orbit.querySelectorAll(".site-footer__badge-orbit-trace"));

      if (!rail || traces.length < 2 || typeof rail.getTotalLength !== "function") {
        return;
      }

      badge = orbit.closest(".site-footer__badge") || orbit;
      styles = window.getComputedStyle(badge);
      total = rail.getTotalLength();
      baseTraceLength = total * (normalizeLength(styles.getPropertyValue("--badge-trace-length"), 8) / 100);
      baseOpacity = normalizeLength(styles.getPropertyValue("--badge-trace-opacity"), 0.67);
      duration = normalizeDuration(styles.getPropertyValue("--badge-orbit-duration"), 6800);
      ...
      function buildTrace(center, length) {
        ...
        for (index = 0; index <= steps; index += 1) {
          point = pointAt(start + (length * index) / steps);
